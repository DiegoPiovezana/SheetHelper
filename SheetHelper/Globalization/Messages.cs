...
namespace SH.Globalization
{
    internal static class Messages
    {
        ...
        internal static string SheetErrorIndex(string sheet)
        {
            return CultureInfo.CurrentCulture.Name switch
            {
                "pt-BR" => $"Erro ao selecionar a aba desejada! Verifique se o índice da aba está correto.",
                _ => $"Error selecting the desired sheet! Please check if the sheet index is correct.",
            };
        }

        internal static string SheetNameNotFind(string sheet)
        {
            return CultureInfo.CurrentCulture.Name switch
            {
                "pt-BR" => $"Não foi possível encontrar a planilha desejada '{sheet}'! Verifique se o nome da planilha está correto.",
                _ => $"Unable to find the desired sheet '{sheet}'! Please check if the sheet name is correct.",
            };
        }
